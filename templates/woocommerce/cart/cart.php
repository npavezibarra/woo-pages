<?php
/**
 * Cart Page - Custom Override for Woo Pages
 *
 * @package Woo_Pages
 */
...
<form class="woocommerce-cart-form" action="<?php echo esc_url(wc_get_cart_url()); ?>" method="post">
    ...
    <table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
        ...
        <!-- DESKTOP TBODY -->
        <tbody class="villegas-cart-desktop">
        ...
        <!-- MOBILE TBODY (Hidden on desktop) -->
        <tbody class="villegas-cart-mobile">
            <?php
            foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                $_product = apply_filters('woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key);
                $product_id = apply_filters('woocommerce_cart_item_product_id', $cart_item['product_id'], $cart_item, $cart_item_key);
                ...
                if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key)) {
                    ...
                    <tr
                        class="woocommerce-cart-form__cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">
                        <td colspan="6" style="padding: 20px 0; border-bottom: 1px solid #e5e7eb;">
                            <div style="display: flex; gap: 15px; align-items: flex-start;">

                                <!-- 1. Thumbnail (20%) -->
                                <div class="villegas-mobile-thumb" style="width: 20%;">
                                    <?php
                                    $thumbnail = apply_filters('woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key);
                                    if (!$product_permalink) {
                                        echo $thumbnail;
                                ...
                                <!-- 2. Details (80%) -->
                                <div class="villegas-mobile-details" style="width: 80%; text-align: left;">
                                    <div class="product-name" style="margin-bottom: 5px; font-weight: 600;">
                                        <?php
                                        if (!$product_permalink) {
                                            echo wp_kses_post($product_name . '&nbsp;');
                                        } else {
                                    ...
                                    <div style="display: flex; align-items: center; gap: 15px;">
                                        ...
                                        <div class="product-subtotal" style="font-weight: bold;">
                                            <?php
                                    ...
                                    <!-- Remove Link -->
                                    <div class="product-remove-link" style="margin-top: 10px;">
                                        <?php
                                        echo apply_filters(
                                            'woocommerce_cart_item_remove_link',
                                            sprintf(
                                                '<a href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s" style="color: #000; text-decoration: none; font-size: 14px; text-transform: uppercase;">REMOVE</a>',
                                                esc_url(wc_get_cart_remove_url($cart_item_key)),
                                                esc_attr(sprintf(__('Remove %s from cart', 'woocommerce'), wp_strip_all_tags($product_name))),
                                                esc_attr($product_id),
                                                esc_attr($_product->get_sku())
